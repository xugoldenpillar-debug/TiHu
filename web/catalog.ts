...
export async function galleryPage(mine = false): Promise<Page> {
  ...
  const trackOptions = `<select id="run-track" name="track">${[
    ...
    .map(
      ([value, label]) =>
        `<option value="${value}" ${value === (q.get("track") ?? "all") ? "selected" : ""}>${label}</option>`,
    )
    .join("")}</select>`;
  return {
    html:
      ...
      (mine && state.quota
        ...
        : "") +
      `<form class="filter-bar" data-filters role="search">${field("run-search", "搜索实验", `<input id="run-search" name="q" type="search" value="${esc(q.get("q"))}" placeholder="题目、模型或作者">`)}${field("run-track", "赛道", trackOptions)}${
        mine
          ? field(
              "run-status",
              "运行状态",
              `<select id="run-status" name="status">${[
                ...
                .map(
                  ([value, label]) =>
                    `<option value="${value}" ${value === (q.get("status") ?? "all") ? "selected" : ""}>${label}</option>`,
                )
                .join("")}</select>`,
            ) +
            field(
              "run-published",
              "公开状态",
              `<select id="run-published" name="published"><option value="all">全部</option><option value="private" ${q.get("published") === "private" ? "selected" : ""}>仅自己可见</option><option value="public" ${q.get("published") === "public" ? "selected" : ""}>已公开</option></select>`,
            )
          : ""
...
export async function leaderboardPage(): Promise<Page> {
  ...
  const filterSelect = (
    ...
    field(
      id,
      label,
      `<select id="${id}" name="${id.replace("board-", "")}">${options.map(([value, text]) => `<option value="${esc(value)}" ${value === current ? "selected" : ""}>${esc(text)}</option>`).join("")}</select>`,
    );
  return {
    html:
      head("社区排行榜", "看大家更喜欢什么，而不是给模型贴上一个智力分数。") +
      `<form class="filter-bar board-filters" data-filters>${filterSelect(
        "board-group",
        "榜单",
        [
          ["works", "作品榜"],
