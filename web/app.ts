...
const routeIcons: Record<string, (size?: number, className?: string) => string> = {
  "/explore": iconCompass,
  "/gallery": iconImage,
...
let renderId = 0;
let navOpen = false;
let navCloseTimer: number | undefined;
...
function navigation(open: boolean, focus = true): void {
  ...
  if (navOpen) {
    ...
    navCloseTimer = window.setTimeout(() => {
      if (!navOpen) backdrop.hidden = true;
    }, 280);
  }
...
let lastAuthUserId: string | undefined | null = "__init__";
...
function shell(path: string): void {
  ...
  if (currentShell && !userChanged) {
    ...
    const links = currentShell.querySelectorAll<HTMLAnchorElement>(".nav-link");
    links.forEach((a) => {
      const href = a.getAttribute("href") ?? "";
      const isLinkActive = href === `#${active}`;
      a.classList.toggle("active", isLinkActive);
      if (isLinkActive) a.setAttribute("aria-current", "page");
