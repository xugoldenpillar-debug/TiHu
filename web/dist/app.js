...
function taskCard(x, i) { const presets = {
    'Pelican on a bicycle': { title: '鹈鹕骑自行车', description: '看似简单的视觉推理：鸟的身体、自行车几何，以及恰到好处的个性。', art: '/art/challenge-pelican.svg', alt: '戴头盔的鹈鹕骑自行车', tone: 'sage' },
    'A tiny living world': { title: '掌心里的小世界', description: '用一个 HTML 页面，构建会呼吸、可互动的迷你生态。', art: '/art/challenge-ecosystem.svg', alt: '被轨道和微小生命环绕的生态星球', tone: 'mist' },
    'The impossible clock': { title: '不太正经的时钟', description: '读时要准确，表达可以天马行空。设计一只出人意料的时钟。', art: '/art/challenge-clock.svg', alt: '漂浮在暖色空间中的俏皮时钟', tone: 'sand' }
}; const p = presets[x.title] || { title: x.title, description: x.description, art: '', alt: '', tone: 'plain' }; return `<article class="card challenge-card ${p.tone}"><a href="#/challenge/${x.id}"><div class="cover">${p.art ? `<img src="${p.art}" alt="${p.alt}" loading="lazy">` : `<div class="generated-mark"><span>${String(i + 1).padStart(2, '0')}</span></div>`}<span class="cover-num">题目 ${String(i + 1).padStart(2, '0')}</span><span class="cover-type">${esc(x.category)}</span></div><div class="card-body"><div class="card-kicker">${esc(x.title)}</div><h3>${esc(p.title)}</h3><p>${esc(p.description)}</p><div class="meta"><span>固定版本 · v${x.current_version}</span><span class="card-arrow">开始挑战 <b>→</b></span></div></div></a></article>`; }
async function explore() { const [tasks, stats] = await Promise.all([api('/challenges'), api('/stats')]); return `<section class="hero"><div class="hero-copy"><div class="eyebrow">ONE SHARED TESTING GROUND</div><h1>把模型放到同一起跑线。</h1><p>选择题目、接入自己的 API Key，让 pi Agent 在隔离环境里完成一次真实构建。先私下预览，再决定是否公开给社区投票。</p><div class="hero-actions"><a class="btn primary" href="#/studio">开始一次实验 →</a><a class="btn light" href="#/leaderboard">看看排行榜</a></div></div><div class="hero-art"><img src="/art/pelican.svg" alt="Pelican riding a bicycle editorial illustration"></div></section><div class="stats"><div class="stat"><span>开放题目</span><strong>${fmt(stats.challenges)}</strong></div><div class="stat"><span>公开作品</span><strong>${fmt(stats.works)}</strong></div><div class="stat"><span>参与模型</span><strong>${fmt(stats.models)}</strong></div></div><div class="section-head"><div><h2>现在可以挑战</h2><p>题目版本固定；不同执行环境不会混榜。</p></div><a class="btn outline small" href="#/new-challenge">创建题目</a></div><div class="grid">${tasks.map(taskCard).join('')}</div>${footer()}`; }
