...
window.addEventListener("tihu-session-change", () => {
    if (draft?.owner !== state.user?.id)
        draft = null;
});
const active = (run) => ["queued", "running"].includes(run.status);
const pathFor = (id) => "/runs/" + encodeURIComponent(id);
const statusLabels = {
    queued: "等待运行",
    running: "正在构建",
    succeeded: "已完成",
...
const numberOrUnknown = (value) => typeof value === "number" && Number.isFinite(value) ? fmt(value) : "未提供";
function quotaHTML() {
    const q = state.quota;
    if (!q)
        return '<p class="help">配额将在提交时由服务器核对。</p>';
...
export async function studioPage() {
    ...
    if (!requested)
        return {
            html: head("新实验", "每次实验使用一个全新的隔离运行环境。") +
                empty("先选择一个任务", "任务规定了模型需要完成什么。你可以从已有挑战开始，也可以创建自己的任务。", '<a class="btn primary" href="#/new-challenge">创建任务</a>') +
                footer(),
        };
    let task = await api("/challenges/" + encodeURIComponent(requested));
    if (!tasks.some((x) => x.id === task.id))
        tasks.unshift(task);
    const selectedVersion = q.get("version") ||
    ...
    return {
        html: `${head("新实验", "冻结任务与配置，运行一次，保留每一步。")}${verificationNotice()}${!keys.length ? '<div class="notice warn"><h3>先连接自己的模型服务</h3><p>TiHu 不提供公共额度。保存 API 连接并发现模型后，才能提交实验。密钥不会写入此页草稿。</p><a class="btn primary" href="#/connections">添加 API 连接</a></div>' : ""}<div class="two"><form class="panel stack" id="studio-form"><section class="stack"><h2>任务与版本</h2>${field("task-search", "查找任务", '<input id="task-search" type="search" placeholder="按标题或描述搜索">')}<div class="help" id="task-search-state" role="status"></div>${field("studio-challenge", "任务", `<select id="studio-challenge" name="challenge">${tasks.map((x) => option(x.id, x.title, d.challenge)).join("")}</select>`)}<button class="btn outline small" type="button" id="more-tasks"${first.next_cursor ? "" : " hidden"}>加载更多任务</button>${field("studio-version", "不可变版本", `<select id="studio-version" name="version_id">${task.versions.map((v) => option(v.id, "v" + v.number, d.version)).join("")}</select>`)}<details class="details" open><summary>原始任务提示词与评审标准</summary><div id="task-contract"></div></details></section><section class="stack"><h2>模型连接</h2>${field("studio-key", "API 连接", `<select id="studio-key" name="key_id">${keys.length ? keys.map((key) => option(key.id, key.label, d.key)).join("") : '<option value="">尚未添加连接</option>'}</select>`)}${field("studio-model", "模型", `<select id="studio-model" name="model">${(selectedKey?.models || []).map((model) => option(model, model, d.model)).join("")}</select>`)}<div id="model-readiness" class="help"></div><a href="#/connections">管理连接与模型列表</a></section><details class="details" id="studio-advanced"${d.advanced ? " open" : ""}><summary>高级设置 · 提示词、思考与 Skills</summary><div class="stack">${field("studio-template", "附加提示词模板", `<select id="studio-template"><option value="">自己编写</option>${prompts.map((prompt) => option(prompt.id, prompt.name, d.template)).join("")}</select>`)}${field("studio-prompt", "附加提示词", `<textarea id="studio-prompt" name="prompt" rows="5" maxlength="5000" placeholder="留空可保持标准赛道">${esc(d.prompt)}</textarea>`, "只追加到原始任务，不会覆盖任务版本。草稿只保留在当前页面会话内，退出账号后清除。")}${field("studio-thinking", "思考级别", `<select id="studio-thinking" name="thinking">${["off", "minimal", "low", "medium", "high", "xhigh", "max"].map((level) => option(level, level === "off" ? "off · 不启用额外思考" : level, d.thinking)).join("")}</select>`, "非 off 设置进入开放赛道；服务商和具体模型是否支持此级别，仍由实际请求决定。")}<fieldset class="field"><legend>Skills · 最多 ${esc(maxSkills)} 个</legend>${skills.length ? `<div class="checkboxes">${skills.map((skill) => `<label class="check"><input type="checkbox" name="skill" value="${esc(skill.id)}"${d.skills.includes(skill.id) ? " checked" : ""}>${esc(skill.name)} <span class="muted">v${esc(skill.current_version || 1)}</span></label>`).join("")}</div>` : '<p class="help">还没有 Skills。<a href="#/skills">上传 Skill</a></p>'}<p class="help">提交时冻结当前文件版本；以后修改 Skill 不会改变这次实验。</p></fieldset></div></details><div id="studio-track" class="notice"></div><div id="studio-quota">${quotaHTML()}</div><label class="check"><input type="checkbox" name="consent" id="studio-consent"${d.consent ? " checked" : ""}>我了解 API 服务商可能产生费用，取消实验无法退回已被接受的请求。</label><p id="studio-ready" class="help" role="status" aria-live="polite"></p><button class="btn primary full" id="studio-submit" type="submit">开始一次性实验</button></form><aside class="panel stack lab-contract"><div class="eyebrow">冻结的实验条件</div><h2>同条件，才可比较。</h2><p>只有任务、任务版本、赛道和环境指纹全部相同，两个实验才会标记为同条件比较。</p><dl class="summary-list">${[
            ...
            .map(([label, value]) => `<div class="summary-row"><dt>${esc(label)}</dt><dd class="break-word">${esc(value ?? "未提供")}</dd></div>`)
            .join("")}</dl><div class="notice warn">这些是资源与请求限制，不是金额上限。请在服务商侧设置消费限额。不会自动重试付费调用。</div><p class="help">标准赛道：无附加提示词、无 Skills，思考级别为 off。开放赛道：只要任一条件改变就进入开放赛道；开放赛道内部配置仍可能不同。</p><a href="#/my-runs">查看已有实验</a></aside></div>${footer()}`,
        mount(root) {
            ...
            const prompt = root.querySelector("#studio-prompt");
            const thinking = select("#studio-thinking");
            const consent = root.querySelector("#studio-consent");
            const submit = root.querySelector("#studio-submit");
            const searchState = root.querySelector("#task-search-state");
            const more = root.querySelector("#more-tasks");
            const remember = () => {
                if (d.owner !== state.user?.id)
                    return;
                Object.assign(d, {
                    ...
                    skills: [
                        ...form.querySelectorAll('input[name="skill"]:checked'),
                    ].map((el) => el.value),
                    consent: consent.checked,
                    template: select("#studio-template").value,
                    advanced: root.querySelector("#studio-advanced").open,
                });
            };
            const syncTask = () => {
            ...
            const readiness = () => {
                ...
                const open = Boolean(prompt.value.trim() || d.skills.length || thinking.value !== "off");
            ...
            const loadTask = async () => {
                ...
                readiness();
                try {
                    const next = await api("/challenges/" + encodeURIComponent(challenge.value), { signal: abort.signal });
            ...
            const searchTasks = async (append) => {
                ...
                try {
                    const result = await api("/challenges?" + params, { signal: abort.signal });
                    if (disposed || serial !== searchRequest)
                        return;
                    const current = challenge.value;
                    const currentLabel = challenge.selectedOptions[0]?.textContent || task.title;
                    if (!append)
                        challenge.innerHTML = option(current, currentLabel, current);
                    const existing = new Set([...challenge.options].map((entry) => entry.value));
                    for (const entry of result.items)
                        if (!existing.has(entry.id)) {
                            challenge.insertAdjacentHTML("beforeend", option(entry.id, entry.title, current));
            ...
            key.addEventListener("change", () => {
                const models = keys.find((entry) => entry.id === key.value)?.models || [];
                const chosen = models.includes(model.value) ? model.value : models[0];
                model.innerHTML = models.length
                    ? models
                        .map((entry) => option(entry, entry, chosen))
                        .join("")
                    : '<option value="">尚无可用模型</option>';
                readiness();
            }, { signal: abort.signal });
            select("#studio-template").addEventListener("change", (ev) => {
                const selected = prompts.find((entry) => entry.id === ev.target.value);
                if (selected)
                    prompt.value = selected.body;
                readiness();
            }, { signal: abort.signal });
            ...
            form.addEventListener("submit", async (ev) => {
                ev.preventDefault();
                readiness();
                if (pending || submit.disabled || !form.reportValidity())
                    return;
                clearFormError(form);
                remember();
                const body = {
...
function metricsHTML(run) {
    const m = run.metrics || {};
    const tokens = m.tokens || {};
    const elapsed = m.elapsed_ms ??
...
function mountPreview(container, run) {
    ...
    const button = container.querySelector("[data-preview-refresh]");
    ...
    const load = async () => {
        ...
        error.hidden = true;
        try {
            const result = await api(pathFor(run.id) + "/preview", {
                signal: abort.signal,
            });
            if (disposed)
                return;
...
export async function runPage(id) {
    ...
    return {
        html: `${head(run.model, `${run.title} · v${run.version} · @${run.username} · ${date(run.created)}`)}<div class="actions" id="run-actions">${actionHTML()}</div><p class="form-error" id="run-action-error" role="alert" hidden></p><div class="run-layout"><div class="stack"><section class="panel" id="run-status">${statusHTML()}</section><div id="run-artifact">${run.status === "succeeded" ? previewHTML(run, "run") + sourceHTML() : ""}</div><section class="panel stack" id="run-events"${run.can_view_events ? "" : " hidden"}><h2>执行进度</h2><p class="help" id="event-state" role="status">正在连接已脱敏的执行日志…</p><ol class="progress-list" id="event-list" aria-label="执行日志"></ol></section><section class="panel" id="run-comments">${commentsHTML(run)}</section><details class="panel details"><summary>不可变配置快照</summary><p class="help">记录提交时的任务、模型、提示词和环境。公开浏览者只可见 Skill 名称与摘要，不会看到 Skill 文件正文。</p><pre class="code">${esc(JSON.stringify(run.snapshot, null, 2))}</pre></details></div><aside class="stack"><section class="panel" id="run-votes">${votesHTML(run)}</section><section class="panel stack"><h2>运行记录</h2><div id="run-metrics">${metricsHTML(run)}</div><dl class="summary-list"><div class="summary-row"><dt>服务商</dt><dd class="mono break-word">${esc(run.provider)}</dd></div><div class="summary-row"><dt>实验 ID</dt><dd class="mono break-word">${esc(run.id)}</dd></div><div class="summary-row"><dt>环境指纹</dt><dd class="mono break-word">${esc(run.environment)}</dd></div></dl><a class="btn outline" href="#/studio?challenge=${encodeURIComponent(run.challenge_id)}&version=${encodeURIComponent(run.version_id)}">以此任务创建新实验</a><p class="help">创建新实验会使用你选择的连接与配置，不会自动复制他人的私有 Skill 或再次调用模型。</p></section></aside></div>${footer()}`,
        mount(root) {
            const abort = new AbortController();
            let disposed = false;
            ...
            const actionError = root.querySelector("#run-action-error");
            const inlineError = (selector, error) => {
            ...
            const loadComments = async () => {
                ...
                const button = root.querySelector('[data-lab-action="load-comments"]');
                if (button)
                    button.disabled = true;
                hideError("[data-comments-error]");
                try {
                    const rows = await api(pathFor(id) + "/comments", {
                        signal: abort.signal,
                    });
            ...
            const refresh = async () => {
                ...
                try {
                    const next = await api(pathFor(id), {
                        signal: abort.signal,
                    });
                    if (disposed)
                        return;
                    ...
                    if (!run.can_view_events) {
                        eventSource?.close();
                        root.querySelector("#run-events").hidden = true;
                    }
                }
                catch (error) {
                    if (!disposed)
                        inlineError("#run-action-error", error);
                }
                finally {
            ...
            const bindCommentForm = () => {
                ...
                form.addEventListener("submit", async (ev) => {
                    ev.preventDefault();
                    if (busy.has("comment") || !form.reportValidity())
                        return;
            ...
            const selectSource = () => {
                if (!source)
                    return;
                const name = root.querySelector("#source-file").value;
                const bytes = Uint8Array.from(atob(source.files[name] ?? ""), (character) => character.charCodeAt(0));
                let text = null;
                ...
                root.querySelector('[data-lab-action="copy-source"]').disabled = text === null;
            };
            const loadSource = async () => {
                if (sourcePending)
                    return;
                sourcePending = true;
                const button = root.querySelector('[data-lab-action="load-source"]');
                button.disabled = true;
                button.textContent = "正在读取…";
                hideError("[data-source-error]");
                try {
                    source = await api(pathFor(id) + "/source", {
                        signal: abort.signal,
                    });
                    if (disposed)
                        return;
                    const names = Object.keys(source.files || {}).sort((a, b) => a === "index.html"
                        ? -1
                        : b === "index.html"
                            ? 1
                            : a.localeCompare(b));
                    if (!names.length)
                        throw new Error("产物没有可读取的源码文件。");
                    const selector = root.querySelector("#source-file");
                    ...
                    selector.innerHTML = names
                        ...
                        .join("");
                    root.querySelector("[data-source-browser]").hidden =
                        false;
                    ...
                    if (!disposed)
                        inlineError("[data-source-error]", error);
                }
                finally {
                    if (!disposed) {
                        sourcePending = false;
            ...
            const report = (commentId) => {
                ...
                const dialog = showDialog(commentId ? "举报评论" : "举报作品", `<p>说明具体问题，管理员会结合作品与评论上下文处理。对同一作品再次举报会更新已有举报。</p><form id="lab-report-form" class="stack">${field("report-reason", "举报原因", `<textarea id="report-reason" name="reason" rows="4" required minlength="5" maxlength="${500 - prefix.length}"></textarea>`)}<button class="btn danger" type="submit">提交举报</button></form>`, (modal) => {
                    const form = modal.querySelector("form");
                    let submitting = false;
                    form.addEventListener("submit", async (ev) => {
                        ev.preventDefault();
                        if (submitting || !form.reportValidity() || disposed)
                            return;
                        const reason = form
                            .querySelector("textarea")
                            .value.trim();
                        if (reason.length < 5) {
                            formError(form, new Error("请至少输入 5 个字符，说明具体问题。"));
                            return;
                        }
            ...
            root.addEventListener("change", (ev) => {
                if (ev.target.id === "source-file")
                    selectSource();
            }, { signal: abort.signal });
            root.addEventListener("click", async (ev) => {
                const button = ev.target.closest("[data-lab-action]");
                ...
                try {
                    if (action === "cancel") {
                        ...
                        const result = await mutate(pathFor(id) + "/vote", "PUT", { kind, active: !run.my_votes?.includes(kind) });
                        if (!disposed) {
                        ...
                        if (await confirmDialog({
                            ...
                            if (disposed)
                                return;
                            await mutate("/comments/" + encodeURIComponent(button.dataset.comment), "DELETE");
                            if (!disposed) {
                                toast("评论已删除");
                                await loadComments();
                            }
                        ...
                        action === "download-all") {
                        if (!source)
                            throw new Error("请先读取源码文件。");
                        if (!selectedSource)
                            throw new Error("当前文件不可读取，请重新加载源码。");
                        const { name, bytes, text } = selectedSource;
                        if (action === "copy-source") {
                            if (text === null)
                                throw new Error("二进制文件不能作为文本复制，请下载原始文件。");
                            if (!navigator.clipboard?.writeText)
                                throw new Error("当前浏览器不能直接复制。请选中源码后复制，或下载当前文件。");
                            ...
                            download(JSON.stringify(source, null, 2), `tihu-${id}-source.json`, "application/json");
                    }
                }
                catch (error) {
                    if (!disposed)
                        inlineError(action.includes("source") || action === "download-all"
                            ? "[data-source-error]"
                            : "#run-action-error", error);
                }
                finally {
                    busy.delete(key);
            ...
            if (run.can_view_events) {
                const status = root.querySelector("#event-state");
                ...
                eventSource.addEventListener("progress", (event) => {
                    ...
                    try {
                        ...
                        const serial = row.id;
                        if (!Number.isFinite(serial) || serial <= lastEventId)
                            return;
                        lastEventId = serial;
...
export async function comparePage() {
    ...
    if (ids.length) {
        if (ids.length !== 2 ||
            ...
            const results = await Promise.allSettled(ids.map((id) => api(pathFor(id))));
            ...
            if (failed)
                ...
                runs = results.flatMap((result) => result.status === "fulfilled" ? [result.value] : []);
                const fields = [
                    ["challenge_id", "任务"],
                    ["version_id", "任务版本"],
                    ["track", "赛道"],
    ...
    return {
        ...
        mount(root) {
            const abort = new AbortController();
            const cleanups = [];
            const compareForm = root.querySelector("#compare-form");
            compareForm.addEventListener("submit", (ev) => {
                ev.preventDefault();
                clearFormError(compareForm);
                if (!compareForm.reportValidity())
                    return;
                const a = root
                    .querySelector("#compare-left")
                    .value.trim();
                const b = root
                    .querySelector("#compare-right")
                    .value.trim();
                if (a === b) {
            ...
            runs.forEach((run, index) => {
                const container = root.querySelector(`[data-preview="compare-${index}"]`);
                if (container)
                    cleanups.push(mountPreview(container, run));
            });
            return () => {
                abort.abort();
