...
/** 绘制晨雾草木叶片 */
function drawLeaf(ctx, size, color, alpha) {
    const w = size * 0.55;
    const h = size * 1.1;
    ctx.fillStyle = color;
    ctx.beginPath();
...
/** 绘制晨曦金芒星粒 */
function drawStar(ctx, size, color, alpha) {
    const r = size * 0.9;
    ctx.fillStyle = color;
    ctx.beginPath();
    ctx.moveTo(0, -r);
...
/** 绘制水滴/晶莹晨露 */
function drawDew(ctx, size, color, alpha) {
    const r = size * 0.75;
    ctx.fillStyle = color;
    ctx.beginPath();
 ...
 */
export function burstParticles(targetOrPoint, options) {
    ...
    const types = options?.types ?? ["leaf", "star", "dew"];
...
export function celebrate(targetOrPoint, options) {
    if (typeof window === "undefined")
        return;
    const now = performance.now();
    if (now - lastCelebrateTime < 350)
        return;
    lastCelebrateTime = now;
    const point = targetOrPoint ?? {
