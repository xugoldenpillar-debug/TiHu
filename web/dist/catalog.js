import { api, mutate, state, esc, fmt, date, duration, head, empty, field, badge, footer, authGate, go, query, toast, errorText, formError, clearFormError, showDialog, closeDialog, confirmDialog, renderVendorBadge, vendorSvgs, } from "./core.js";
const challengeCopy = {
    "Pelican on a bicycle": {
        title: "鹈鹕骑自行车",
        description: "鸟的身体、自行车的几何，还有生动的骑行动画。用 SVG 2D 动画看看模型如何理解结构与运动节奏。",
    ...
    "鹈鹕骑行": {
        title: "鹈鹕骑行",
        description: "经典视觉推理题目。用纯 SVG 绘制鹈鹕骑自行车的 2D 循环动画，考察模型对生物骨骼、机械几何与运动韵律的理解。",
        art: "/art/challenge-pelican.svg",
        alt: "戴着头盔的鹈鹕骑自行车",
        rubric: [
            "一眼能认出的鹈鹕与清晰鸟喙",
    ...
    "Qin Shi Huang on a polar bear": {
        title: "秦始皇骑北极熊",
        description: "千古一帝与极地巨兽的奇幻碰撞。用 SVG 考察模型对复杂人物服饰、动物骨骼与 2D 动态节奏的综合表现力。",
...
export function generateDynamicCoverSVG(title, category = "SVG") {
...
export function resolveChallengeVisual(item) {
    ...
    if (normTitle.includes("polar bear") ||
    ...
    if (normTitle.includes("ecosystem") ||
...
function initHeroShowcase(root) {
    const tabs = root.querySelectorAll("[data-hero-model]");
    const img = root.querySelector("#hero-pelican-stage");
    const tagEl = root.querySelector("#hero-pelican-tag");
    const techEl = root.querySelector("#hero-pelican-tech");
    if (!tabs.length || !img)
        return () => { };
    let currentIndex = 0;
    let timer = null;
    ...
    const selectModel = (index) => {
        ...
        if (tagEl)
            tagEl.innerHTML = `${icons.sparkles} <span>${esc(model.tag)}</span>`;
        if (techEl)
            techEl.innerHTML = `<span class="hero-pulse-dot"></span> <span>${esc(model.tech)}</span>`;
    };
    ...
    `,
        mount(root) {
            ...
            root.querySelector("#upload-work-btn")?.addEventListener("click", () => {
                ...
                showDialog("上传自制作品", `<form id="upload-work-form" class="stack">
            ...
            <div class="actions">
              ...
              <button class="btn primary" type="submit">提交并进行安全审计</button>
                    ...
                    form.addEventListener("submit", async (e) => {
                        e.preventDefault();
                        const fileInput = form.querySelector("#uw-file");
                        const file = fileInput?.files?.[0];
                        if (!file)
                            return;
                        const titleVal = form.querySelector("#uw-title")?.value.trim() || "";
                        ...
                        try {
                            ...
                            fd.append("file", file);
                            const res = await api("/runs/upload", {
                                method: "POST",
                                body: fd,
                            });
                            closeDialog();
