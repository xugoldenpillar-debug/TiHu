...
export function inferFieldIcon(id) {
    const lower = id.toLowerCase();
    if (lower.includes("search") || lower === "q")
        return icons.search;
    if (lower.includes("email") || lower.includes("mail"))
        return icons.mail;
    if (lower.includes("password"))
        return icons.lock;
    if (lower.includes("username") || lower.includes("author"))
        return icons.user;
    if (lower.includes("key") || lower.includes("secret") || lower.includes("token"))
        return icons.key;
    if (lower.includes("url") || lower.includes("base") || lower.includes("link"))
        return icons.link;
    if (lower.includes("prompt"))
        return icons.sparkles;
    if (lower.includes("rubric") || lower.includes("file"))
...
export function field(id, label, control, help = "", icon = "") {
...
let closeTimer;
export function closeDialog(immediate = false) {
...
export function authenticate(mode = "login") {
    ...
    const subtitle = forgot
        ...
        : register
            ? "保存自己的模型连接，开始一次私有实验。"
            : "继续你的模型实验。";
    const tabsHtml = !forgot && state.config.registration
