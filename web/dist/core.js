...
export class ApiError extends Error {
    ...
    fields;
    retryAfter;
    constructor(code, status, fields = [], retryAfter = null) {
        super(code);
        this.code = code;
        this.status = status;
        this.fields = fields;
...
export async function api(path, init = {}) {
    ...
    if (!response.ok) {
        ...
        throw new ApiError(typeof body.detail === "string" ? body.detail : "request_failed", response.status, Array.isArray(body.errors)
...
export const fmt = (value) => numberFormat.format(Number.isFinite(value) ? value : 0);
export const date = (value) => Number.isFinite(value)
...
export function authGate() {
    return {
        html: head("先登录，开始你的实验", "连接、提示词和未公开的作品仅对你可见。") +
            empty("选好题目，再接上自己的模型。", "注册后可以保存 API 连接；真实 Key 不进入模型沙箱。", `${state.config.registration ? '<button class="btn primary" data-global="register">创建账号</button>' : ""} <button class="btn outline" data-global="login">登录</button>`),
    };
}
...
export function showDialog(title, content, mount) {
    closeDialog();
    const dialog = document.querySelector("#modal");
    dialog.className = "";
...
export function confirmDialog(options) {
    ...
    const dialog = showDialog(options.title, `<div class="dialog-body">${options.body}</div><div class="actions"><button class="btn outline" type="button" data-cancel>暂不操作</button><button class="btn ${options.danger ? "danger" : "primary"}" type="button" data-confirm>${esc(options.confirm)}</button></div>`);
...
export function authenticate(mode = "login") {
    ...
    form.addEventListener("submit", async (event) => {
        ...
        clearFormError(form);
        try {
            const body = { email: String(data.get("email") ?? "") };
            if (!forgot)
                body.password = String(data.get("password") ?? "");
            if (register)
                body.username = String(data.get("username") ?? "");
            await mutate("/auth/" + (forgot ? "forgot" : register ? "register" : "login"), "POST", body);
            closeDialog();
            if (forgot)
