export type IconFn = (size?: number, className?: string) => string;
 ...
 */
export function createIcon(paths: string, defaultSize = 18): IconFn {
  return (size = defaultSize, className = "") =>
    `<svg xmlns="http://www.w3.org/2000/svg" width="${size}" height="${size}" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.75" stroke-linecap="round" stroke-linejoin="round"${className ? ` class="${className}"` : ""} aria-hidden="true">${paths}</svg>`;
}
...
export const iconFlask = createIcon(
  '<path d="M10 2v7.31L4.15 19.34A2 2 0 0 0 5.86 22h12.28a2 2 0 0 0 1.71-2.66L14 9.31V2"/><path d="M8.5 2h7"/><path d="M7 16h10"/>',
);
...
export const iconPlusCircle = createIcon(
...
export const iconToken = createIcon(
  '<circle cx="12" cy="12" r="9"/><path d="M12 6v12"/><path d="M15 9.5a3.5 3.5 0 0 0-5 0c0 2 5 2 5 5a3.5 3.5 0 0 1-6 0"/>',
);
export const iconPrompt = createIcon(
  '<path d="M4 4h16c1.1 0 2 .9 2 2v12c0 1.1-.9 2-2 2H4c-1.1 0-2-.9-2-2V6c0-1.1.9-2 2-2z"/><path d="m8 10 3 2-3 2"/><path d="M13 14h3"/>',
);
...
export const iconBranch = createIcon(
 ...
 */
export const vendorSvgs: Record<string, string> = {
...
export function detectVendor(modelOrProvider: string): { key: string; name: string } {
  const s = (modelOrProvider || "").toLowerCase();
  if (s.includes("claude") || s.includes("anthropic")) return { key: "anthropic", name: "Anthropic" };
  if (s.includes("gpt") || s.includes("openai") || s.includes("o1") || s.includes("o3")) return { key: "openai", name: "OpenAI" };
  if (s.includes("deepseek")) return { key: "deepseek", name: "DeepSeek" };
  if (s.includes("gemini") || s.includes("google")) return { key: "gemini", name: "Google Gemini" };
  if (s.includes("mistral") || s.includes("codestral")) return { key: "mistral", name: "Mistral AI" };
  if (s.includes("llama") || s.includes("meta")) return { key: "meta", name: "Meta Llama" };
  if (s.includes("groq")) return { key: "groq", name: "Groq" };
  if (s.includes("ollama")) return { key: "ollama", name: "Ollama" };
  return { key: "generic", name: "AI Model" };
}

export function renderVendorBadge(modelOrProvider: string, extraClass = ""): string {
