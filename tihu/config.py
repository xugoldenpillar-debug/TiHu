...
class Settings:
    ...
    concurrency: int = field(default_factory=lambda: env_int('WORKER_CONCURRENCY', 10, 1, 32))
    run_seconds: int = field(default_factory=lambda: env_int('RUN_SECONDS', 900, 20, 7200))
    max_calls: int = field(default_factory=lambda: env_int('MAX_MODEL_CALLS', 50, 1, 200))
    max_output_tokens: int = field(default_factory=lambda: env_int('MAX_OUTPUT_TOKENS', 16384, 256, 65536))
    daily_runs: int = field(default_factory=lambda: env_int('DAILY_RUNS', 20, 1, 1000))
    user_active: int = field(default_factory=lambda: env_int('USER_ACTIVE_RUNS', 5, 1, 10))
