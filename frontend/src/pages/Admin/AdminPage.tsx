...
import { Shield, Users, BookOpen, AlertOctagon } from "lucide-react";
  ...
  if (user?.role !== "admin") {
    return (
      <div className="flex flex-col items-center justify-center p-16 max-w-md mx-auto text-center gap-3">
        ...
        <h2 className="text-xl font-bold text-slate-900">无权限访问管理中心</h2>
        <p className="text-xs text-slate-500">此页面仅向平台管理员开放。</p>
  ...
  return (
    <div className="flex flex-col gap-6 max-w-5xl mx-auto w-full pb-16 animate-in fade-in duration-200">
      <div className="flex items-center gap-3">
        ...
        <div>
          <h1 className="text-xl font-bold text-slate-900">平台安全与治理中心 (Admin)</h1>
          <p className="text-xs text-slate-500">管理创作者账号状态、题目归档与内容合规审查。</p>
        </div>
      </div>
      ...
      {loading ? (
        ...
        <Card className="p-3 bg-white border-slate-200 divide-y divide-slate-100">
          {users.map((u) => (
            <div key={u.id} className="flex items-center justify-between py-3 px-2">
              <div className="flex flex-col gap-0.5">
                <div className="flex items-center gap-2">
                  ...
                  {u.verified ? (
                    <span className="text-[10px] text-emerald-600 font-medium">已验证</span>
                  ) : (
                    <span className="text-[10px] text-amber-600 font-medium">未验证</span>
                  )}
                </div>
        ...
        <Card className="p-3 bg-white border-slate-200 divide-y divide-slate-100">
          {challenges.map((c) => (
            <div key={c.id} className="flex items-center justify-between py-3 px-2">
              <div className="flex flex-col gap-0.5 max-w-xl">
                <div className="flex items-center gap-2">
                  <span className="font-bold text-xs text-slate-900">{c.title}</span>
                  <Badge variant="neutral" size="sm">v{c.current_version}</Badge>
        ...
        <Card className="p-3 bg-white border-slate-200 divide-y divide-slate-100">
          {prompts.map((p) => (
            <div key={p.id} className="flex flex-col gap-1.5 py-3 px-2">
              <div className="flex items-center justify-between">
                <span className="font-bold text-xs text-slate-900">{p.name}</span>
                <span className="text-[11px] text-slate-400">作者: {p.author}</span>
              </div>
              <pre className="p-2.5 rounded-lg bg-slate-50 border border-slate-100 text-xs text-slate-700 font-mono whitespace-pre-wrap max-h-28 overflow-y-auto">
                {p.body}
              </pre>
            </div>
          ))}
